...
$titulo = empty($_GET['id'])? 'Registrar curso' : 'Modificar curso';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    ...
    <div>
        ...
        <div class="row justify-content-center">
            <form action="index.php?page=cursos&view=save" method="POST" class="col-8">
            <?php
                if(!empty($_GET['id'])){
                    echo '<input type="hidden" name="id" id="id" value="'.$curso->get('id').'">';
                }
            ?>
                    <a style="color: white;display: flex; align-items: center; justify-content: end;" href="index.php?page=cursos"><button type="button" class="btn btn-light">Volver</button></a>
                <div style="color: white;">
                    <label for="nombre">Nombre</label>
                    <input class="form-control" type="text" name="nombre" id="nombre" value="<?php echo $curso->get('nombre') ?>" required>
                </div>
                <div style="color: white;">
                    <label for="docente_id">Docente_id</label>
