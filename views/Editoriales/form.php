...
$titulo = empty($_GET['id'])? 'Registrar docente' : 'Modificar docente';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <div>
        ...
        <form action="index.php?page=docentes&view=save" method="POST" class="col-8">
        <?php
            if(!empty($_GET['id'])){
                echo '<input type="hidden" name="id" id="id" value="'.$docente->get('id').'">';
            }
        ?>
            <a style="color: white;display: flex; align-items: center; justify-content: end;" href="index.php?page=docentes"><button type="button" class="btn btn-light">Volver</button></a>
